...
module.exports = function destroyOneRecord (req, res) {
  ...
  var query = Model.findOne(crite_obj, queryOptions.populates).meta(queryOptions.meta);
  query.exec(function foundRecord (err, record) {
    ...
    Model.destroy(crite_obj).exec(function destroyedRecord (err) {
      ...
      if (req._sails.hooks.pubsub) {
        ...
        let cirte_object_primary = criteria[Model.primaryKey];
        cirte_object_primary.userId = req.session.userId;
        Model._publishDestroy(cirte_object_primary, !req._sails.config.blueprints.mirror && req, {previous: record});
        if (req.isSocket) {
